...
//formatting current video time to hh:mm:ss format
...
//retrieve current video time for display
videoElement.addEventListener('timeupdate', () => {
  videoTime.innerText = formatTime(videoElement.currentTime);
});
...
function muteUnmute(){
  console.log("mute/unmute"); //to check if its working properly
...
fullscreenButton.addEventListener("click", () => {
  videoElementWrapper.classList.toggle("fullscreen");
  if (document.fullscreenElement) { //if the video is already in full screen mode
    document.exitFullscreen();
  } 
  videoElementWrapper.requestFullscreen();
});

backButton.addEventListener("click", () => {
  videoElement.currentTime -= 5; //subtract 5 seconds from current video time
});

forwardButton.addEventListener("click", () => {
  videoElement.currentTime += 5; //adds 5 seconds to current video time
});

//assigning icons
fullscreenButton.style.backgroundImage = "url('./icons/fullScreen.svg')";
backButton.style.backgroundImage = "url('./icons/rewind.svg')";
